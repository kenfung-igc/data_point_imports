...
def train_asset_type_clf(points, clf=None):
    ...
    test_df = test_df.merge(known_points, left_index=True, right_index=True).merge(y_test, left_index=True, right_index=True)
...
def predict_asset_type(clf, points):
    ...
    print(f"Fail to predict asset type for {len(points[~points['matched']].index)} points")
    return clf
